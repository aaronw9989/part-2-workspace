...
public class OrderTaxTest {
  ...
  public void init() {
    ...
    // Online tax = 0, US tax = 0 Euro tax = 3.4
    ...
    // Online tax = 0, US tax = 10.00, Euro tax = 25.40
  }
  ...
  public void testTaxOnlineOrder() {
    Order order = new Order("online-order-2", Location.ONLINE);
  ...
  public void testTaxUSAOrder() {
    Order order = new Order("us-order-1", Location.USA);
    order.processCart(cart1);
    assertEquals(0, order.getTax(), TOLERANCE);
    order.processCart(cart2);
